...
var eosVoter = class {
    ...
    storeSelectedBPs(bps) {
        localStorage.setItem('selectedBPs', '');
    ...
    refreshBPs() {
        ...
        console.log('this.populate ', this.populateBPs())
    ...
    buildTable(res) {
       ...
       var eos_candidate= '';
        ...
        for (var i = 0; i < sorted.length; i++) {
            ...
            let  bPUrl = (row.url) ? this.trimUrl(row.url)+'/bp.json' : '';
            //console.log(bPUrl);

            console.log(this.doesFileExist(bPUrl));

            if (this.doesFileExist(bPUrl)) {
              fetch(bPUrl)
                .then(function(response) {
                  return response.json();
                })
                .then(function(json) {
                  console.log("parsed json", json.org.candidate_name);
                  eos_candidate = json.org.candidate_name;
                })
                .catch(function(ex) {
            ...
            //    } else {
            //        eos_candidate123 = object.producer_account_name;
            //      console.log(object);
            //    }
           //   });
            ...
            /* fetch(bPUrl)
              ...
              .then(json => eos_candidate123 = myJson.producer_account_name)
              .catch(error => console.log(error)); */

            table.append(tr);
            tr.append(this.addTd('<input name="bpVote" type="checkbox" value="' + row.owner + '" ' + (this.bpShouldBeSelected(row.owner) ? 'checked disabled' : '') + ' >'));
            
              tr.append(this.addTd(eos_candidate));
